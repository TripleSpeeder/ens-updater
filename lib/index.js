...
module.exports = function Updater() {
    ...
    // module variables
    let controllerAddress, resolver, registry, verbose, node, web3, ensName
    ...
    async function verifyInterface(interfaceName) {
        ...
        if (!EIPSupport) {
            throw new Error(`Resolver contract does not support "${interfaceName}" standard`)
        }
        let support = await resolver.supportsInterface(ResolverInterfaces[interfaceName])
        if (!support) {
            throw new Error(`Resolver contract does not support required interface "${interfaceName}"`)
        }
    }
