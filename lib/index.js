...
module.exports = function Updater() {
    ...
    // module variables
    let resolver, registry, registrar, controllerAddress, verbose, node, web3, ensName, dryrun
    ...
    async function execute(contract, method, ...args) {
        ...
        // Add transaction options only for non-constant methods
        if(!isMethodConstant(contract, method)) {
            const transactionOptions = {
                from: controllerAddress
                // TODO: Add additional options like gas parameters etc. here
            }
            args.push(transactionOptions)
        }

        // Interact with contract and return promise
        return func(...args)
    }
    ...
    async function getAddress() {
        ...
        try{
            const address = await execute(resolver, 'addr', node)
            return address
        }catch(error){
    ...
    async function getContenthash() {
        ...
        try{
            const currentContenthash = await execute(resolver, 'contenthash', node)
    ...
    async function listInterfaces() {
        ...
        try {
            for (const interface of Object.keys(ResolverInterfaces)) {
                const support = await execute(resolver, 'supportsInterface', ResolverInterfaces[interface])
